...
@app.route('/digit-transitions')
def digit_transitions():
    df = load_csv_data()
    if df.empty:
        return "No data available to analyze.", 404

    selected_provider = request.args.get('provider', 'all')
    selected_month = request.args.get('month', '')

    # --- Filtering Logic (same as day-to-day) ---
    provider_options = ['all'] + sorted([p for p in df['provider'].dropna().unique() if p])
    month_options = sorted(df['date_parsed'].dropna().dt.strftime('%Y-%m').unique(), reverse=True)
    ...
    for start_digit in "0123456789":
        viz_data[start_digit] = {'position_data': []}
        for pos in range(4):
            pos_transitions = digit_transitions.get(start_digit, {}).get(pos, {})
            if not pos_transitions:
                viz_data[start_digit]['position_data'].append([])
                continue

            total_count = sum(pos_transitions.values())
            
            processed_pos_data = []
            for next_digit, count in pos_transitions.items():
                percentage = (count / total_count) * 100 if total_count > 0 else 0
                processed_pos_data.append({'next_digit': next_digit, 'count': count, 'percentage': percentage})
            
            # Sort by percentage and take top 3 for a clean view
            processed_pos_data.sort(key=lambda x: x['percentage'], reverse=True)
            viz_data[start_digit]['position_data'].append(processed_pos_data[:3])

    return render_template(
        'digit_transitions.html',
        last_updated=time.strftime('%Y-%m-%d %H:%M:%S'),
        provider_options=provider_options,
        provider=selected_provider,
        month_options=month_options,
        selected_month=selected_month,
        transition_viz=viz_data
    )
