...
def decision_helper():
    ...
    final_picks = _calculate_final_picks(adv, smart, ml)
    if not final_picks:
...
def _get_provider_options(df):
    """Get valid provider options from dataframe"""
    valid_providers = [p for p in df['provider'].dropna().unique() if p]
    return ['all'] + sorted(valid_providers)

def _get_fallback_predictions(df):
    """Generate fallback predictions from frequency analysis"""
    all_nums = []
    for col in ['1st_real', '2nd_real', '3rd_real']:
        column_values = df[col].tail(100).astype(str)
        for value in column_values:
    ...
    if all_nums:
        freq = Counter(all_nums).most_common(10)
        return [(num, 1.0, 'frequency') for num, count in freq]
    return []

def _calculate_final_picks(adv, smart, ml):
    """Calculate final picks using voting system"""
    votes = {}
    for num, score, _ in adv + smart + ml:
    ...
    return [(num, min(count * 25, 95)) for num, count in sorted_votes[:5]]

def _get_backup_numbers(adv, smart, ml, final_picks):
    """Get backup numbers from all predictions"""
    all_candidates = set()
    for prediction_list in [adv, smart, ml]:
        for num, _, _ in prediction_list:
            all_candidates.add(num)
    
    final_nums = {num for num, _ in final_picks}
    backup_candidates = all_candidates - final_nums
    return list(backup_candidates)[:10]
...
def decision_helper():
    ...
    # Get predictions
    adv = advanced_predictor(df, provider, 200) or []
    smart, ml = [], []  # Simplified for now
    adv = adv[:10] if adv else []
    
    # Use fallback if no predictions
    if not adv and not smart and not ml:
        logger.warning("All predictors returned empty! Using fallback...")
        adv = _get_fallback_predictions(df)
        logger.info(f"Fallback created {len(adv)} predictions")
    ...
    backup_numbers = _get_backup_numbers(adv, smart, ml, final_picks)
...
def _load_prediction_data():
    """Load and filter prediction tracking data"""
    pred_file = "prediction_tracking.csv"
    if os.path.exists(pred_file):
        pred_df = pd.read_csv(pred_file)
        return pred_df[pred_df['hit_status'] != 'pending']
    return pd.DataFrame()

def _calculate_prediction_stats(completed_df):
    """Calculate prediction statistics"""
    total_predictions = len(completed_df)
    exact_matches = len(completed_df[completed_df['hit_status'] == 'EXACT'])
    three_digit_matches = len(completed_df[completed_df['hit_status'] == '3-DIGIT'])
    ...
    if total_predictions > 0:
        exact_score = exact_matches * 100
        three_digit_score = three_digit_matches * 75
        weighted_score = exact_score + three_digit_score
        stats['overall_accuracy'] = weighted_score / total_predictions
    
    return stats

def _get_recent_predictions(completed_df):
    """Get recent predictions data"""
    if not completed_df.empty:
        return completed_df.tail(10).to_dict('records')
    return []
...
def learning_dashboard():
    ...
    completed = _load_prediction_data()
    stats = _calculate_prediction_stats(completed)
    methods = learner.get_best_methods(top_n=10)
    recent_predictions = _get_recent_predictions(completed)
