...
def power_dashboard():
    ...
    try:
        ...
        try:
            ...
            # Fallback functions
            def enhanced_predictor(df, provider, lookback):
                return advanced_predictor(df, provider, lookback)
            class ConfidenceScorer:
                def batch_score(self, preds, numbers):
                    return preds
            class AdaptiveLearner:
                learning_data = {'method_weights': {}}
                def get_update_stats(self):
                    return {}
            class AutoUpdater:
                def get_update_stats(self):
                    return {'total_updates': 0, 'last_update': 'N/A', 'avg_rows_per_update': 0}
            df = df[df['provider'] == provider]
