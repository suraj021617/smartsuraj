...
def predict_top_5(draws, mode="combined", provider=None):
    ...
    # 🔹 Provider-specific bias (ensures providers differ)
    if provider and provider != "all":
        prov_key = re.sub(r"[^a-z0-9]", "", str(provider).lower())
        for cand in list(results.keys()):
            # Improved provider bias logic using hash-based matching
            if prov_key and len(prov_key) > 0:
                # Create a more sophisticated hash for provider matching
                provider_hash = hash(prov_key) % 10000
                provider_digits = str(provider_hash).zfill(4)
                
                # Check for digit overlap between provider hash and candidate
                overlap = len(set(provider_digits) & set(cand))
                if overlap >= 2:  # At least 2 matching digits
                    results[cand] *= 1.1
                    reason_map[cand].add(f"provbias-{prov_key}")
                elif overlap == 1:  # Single digit match gets smaller boost
                    results[cand] *= 1.05
                    reason_map[cand].add(f"provbias-{prov_key}")
