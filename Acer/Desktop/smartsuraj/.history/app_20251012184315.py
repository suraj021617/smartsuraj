...
def advanced_predictor(df, provider=None, lookback=200):
    ...
    for num in candidate_pool:
        ...
        score += hot_score * 1.5 # Give more weight to hot digits
        reasons.append(f"🔥 Hot Digits ({hot_score:.1f})")
        pair_bonus = 0.0
        for i in range(3):
            pair = num[i : i + 2]
            pair = num[i: i + 2]
        ...
        tval = transitions.get(num, 0)
        if isinstance(tval, (int, float)) and tval > 0:
            trans_val = float(tval) * 0.5
            score += trans_val
            reasons.append(f"🔄 Transition ({trans_val:.1f})")

        if provider and provider != "all":
            if provider_multiplier > 1.1:
                reasons.append(f"🎯 Provider Bias")
...
def smart_auto_weight_predictor(df, provider=None, lookback=300):
    ...
    for num in candidate_pool:
        ...
        scored.append((num, total, f"Auto-tuned formula (H:{w_hot:.1f}, P:{w_pair:.1f}, T:{w_trans:.1f})"))

    scored.sort(key=lambda x: x[1], reverse=True)
    top_5 = [(num, round(score, 3), reason) for num, score, reason in scored[:5]]
    top_5 = [(num, round(score, 2), reason) for num, score, reason in scored[:5]]
    return top_5
...
def ml_predictor(df, lookback=500):
    ...
    for num in candidate_pool:
        ...
        scored.append((num, float(pred_score), "Learned from historical sequences"))
...
def get_day_to_day_predictions_with_reasons(today_numbers, patterns, recent_history):
    ...
    for today_num in set(today_numbers):
        ...
        # Reason 2: Check for digit transitions
        for i, digit in enumerate(today_num):
            if digit in digit_transitions and i in digit_transitions[digit]:
                for next_digit, count in digit_transitions[digit][i].items():
                    # This is a weaker signal, so we generate candidates
                    # (This part is complex and better handled inside a dedicated predictor)
                    pass # We will rely on the stronger sequence patterns for clarity
