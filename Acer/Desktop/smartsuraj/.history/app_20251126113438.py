...
def slider_dashboard():
    ...
    try:
        ...
        return render_template('slider_dashboard.html', results=[], recent_winners=[], learning_progress=[])l])) == 4 and str(row[col]).isdigit():
                        recent_winners.append({
                            'date': row['date_parsed'].strftime('%Y-%m-%d'),
                            'day': row['date_parsed'].strftime('%A'),
                            'provider': 'Lottery',
...
if __name__ == '__main__':
    ...
    app.run(debug=debug_mode)', 'exact': 6, '3_digit': 32, 'accuracy_rate': 58}
    ]
