...
import functools
import time
from learning_engine import predict_with_model, extract_ml_features
...
def cached_data_operation(func):
    """Decorator for caching data operations"""
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        cache_key = f"{func.__name__}_{hash(str(args) + str(kwargs))}"
        if cache_key in _data_cache:
            cached_time, cached_result = _data_cache[cache_key]
            if time.time() - cached_time < 300:  # 5 minute cache
                return cached_result
        result = func(*args, **kwargs)
        _data_cache[cache_key] = (time.time(), result)
        return result
    return wrapper

@cached_data_operation
def load_data():
...
def ensemble_predict(module_results, df=None):
    """Combine all module predictions with improved weighted voting and diversity scoring, enhanced with ML model"""
    votes = Counter()
    sources = []
    total_conf = 0
    module_weights = {
        ...
        'frequency': 0.9,
        'missing': 0.8,
        'day_to_day': 1.1,  # Higher weight for day-to-day patterns
        'pattern': 0.7,
        'empty_box': 0.6
    }
    ...
    for nums, conf, src in module_results:
        ...
        weight = module_weights.get(src, 0.8)  # Default weight
        adjusted_conf = conf * weight

        for num in unique_nums:
            votes[num] += adjusted_conf
        sources.append(src)
        total_conf += adjusted_conf

    # Diversity bonus: slightly boost numbers that appear in multiple modules
    diversity_bonus = {}
    for num in votes:
        module_count = sum(1 for nums, _, _ in module_results if num in nums)
        diversity_bonus[num] = module_count * 0.05  # Small bonus per module

    # Apply diversity bonus
    for num in votes:
        votes[num] += diversity_bonus.get(num, 0)

    # Get initial top candidates
    candidates = votes.most_common(10)  # Get more candidates for ML scoring

    # Apply ML model scoring if available
    ml_boosted = {}
    if df is not None:
        for num, base_score in candidates:
            # Create dummy row for feature extraction
            dummy_row = {
                'predicted_numbers': num,
                'hot_cold_score': 0.5,  # Placeholder
                'confidence_score': base_score / 10,  # Normalize
                'gap_pattern': '+0,+0',
                'pattern_source': '+'.join(sources)
            }
            try:
                ml_conf = predict_with_model(extract_ml_features(dummy_row, df), df)
                if ml_conf is not None:
                    # Boost score based on ML confidence
                    ml_boost = ml_conf * 2.0  # Scale up ML influence
                    ml_boosted[num] = base_score + ml_boost
                else:
                    ml_boosted[num] = base_score
            except:
                ml_boosted[num] = base_score
    else:
        ml_boosted = dict(candidates)

    # Top 6 unique numbers by ML-boosted votes
    top_6 = [n for n, _ in sorted(ml_boosted.items(), key=lambda x: x[1], reverse=True)][:6]
    avg_conf = total_conf / len(module_results) if module_results else 0
    pattern_source = '+'.join(sources) + ('+ML' if df is not None and os.path.exists('master_model.pkl') else '')
...
def predict_tomorrow():
    ...
    print("\n🎯 Ensemble prediction...")
    predicted, confidence, pattern_source = ensemble_predict(module_results, df)
