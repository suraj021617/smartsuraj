...
def advanced_analytics():
    ...
    day_predictions = {}
    for day in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']:
        day_df = df[df['date_parsed'].dt.day_name() == day]
        if not day_df.empty:
            day_predictions[day] = ml_predictor(day_df, lookback=100)
...
def smart_auto_weight_predictor(df, provider=None, lookback=300):
    ...
    if cache_key in _smart_model_cache:
        model, scaler, w_hot, w_pair, w_trans, digit_counts, pair_counts, transitions = _smart_model_cache[cache_key]
    else:
        ...
        _smart_model_cache[cache_key] = (model, scaler, w_hot, w_pair, w_trans, digit_counts, pair_counts, transitions)
...
def ultimate_predictor():
    df = load_csv_data()
    selected_provider = request.args.get('provider', 'all').lower().strip()
    
    if df.empty:
        return render_template('ultimate_predictor.html', error="No data available")
    
    # --- FIX: Correctly generate and filter provider options ---
...
def accuracy_dashboard():
    ...
    for idx, row in pred_df.iterrows():
        if row['hit_status'] == 'pending':
            try:
                # --- FIX: Handle simple 'YYYY-MM-DD' date format ---
...
def best_predictions():
    ...
    for num, data in all_predictions.items():
        consensus_score = data['count'] * (data['total_score'] / data['count'])
        confidence = (data['count'] / 3) * 100  # 3 predictors
        final_predictions.append({
            'number': num,
