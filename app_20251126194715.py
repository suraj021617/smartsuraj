...
def power_dashboard():
    ...
    try:
        ...
        if advanced_available:
            # Try to use optional advanced modules; fall back gracefully if unavailable
            try:
                from utils.power_predictor import enhanced_predictor
                from utils.confidence_scorer import ConfidenceScorer
                from utils.adaptive_learner import AdaptiveLearner
                ...
                learner = AdaptiveLearner()
                adaptive_weights = learner.learning_data.get('method_weights', {'advanced': 0.25, 'smart': 0.25, 'ml': 0.25, 'pattern': 0.25})
                updater = AutoUpdater()
                update_stats = updater.get_update_stats()
                feature_importance = []
            except Exception as e:
                logger.warning(f"Enhanced modules not available or failed: {e}; falling back to basic predictions.")
                ...
                update_stats = {'total_updates': 0, 'last_update': 'Unavailable', 'avg_rows_per_update': 0}
                feature_importance = []
        else:
...
if __name__ == "__main__":
    app.run(debug=True, host='127.0.0.1', port=5000)error("No votes collected!")
            return render_template('decision_helper.html', error="No predictions available", final_picks=[], reasons=[], provider_options=provider_options, provider=provider, next_draw_date='', provider_name='', backup_numbers=[])
