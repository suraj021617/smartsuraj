...
def load_csv_data():
    ...
    # Remove duplicates by date+provider AND by prize numbers to prevent showing same results multiple times
...
def index():
    ...
    # Generate past results based on actual recent data
    recent_numbers = []
    if not df.empty:
    ...
    if not recent_numbers:
        recent_numbers = ['1234', '5678', '9012', '3456', '7890']
    
    damacai_3plus3d = {
        'draw_no': cards[0].get('draw_info', 'DMC0000') if cards else 'DMC0000',
        'date': selected_date,
        'numbers': recent_numbers[:3] if len(recent_numbers) >= 3 else ['123', '456', '789']
    }

    sports_toto_games = [
        {'name': 'Toto 5D', 'draw_no': 'T5D001', 'date': selected_date, 'result': recent_numbers[0] + '5' if recent_numbers else '12345'},
        {'name': 'Toto 6D', 'draw_no': 'T6D002', 'date': selected_date, 'result': recent_numbers[1] + '21' if len(recent_numbers) > 1 else '654321'},
        {'name': 'Toto Lotto', 'draw_no': 'TL003', 'date': selected_date, 'result': f"{recent_numbers[0][:2]} {recent_numbers[1][:2]} {recent_numbers[2][:2]}" if len(recent_numbers) >= 3 else '12 23 34'}
    ]

    magnum_life = {
        'draw_no': cards[0].get('draw_info', 'ML0000') if cards else 'ML0000',
        'date': selected_date,
        'numbers': [num[:2] for num in recent_numbers[:8]] if len(recent_numbers) >= 8 else ['01', '05', '12', '18', '22', '27', '33', '39'],
        'life_number': recent_numbers[0][0] if recent_numbers else '07'
    }

    magnum_jackpot_gold = {
        'draw_no': cards[0].get('draw_info', 'MJG0000') if cards else 'MJG0000',
        'date': selected_date,
        'numbers': [num[:2] for num in recent_numbers[:5]] if len(recent_numbers) >= 5 else ['03', '11', '19', '25', '32'],
...
def frequency_analyzer():
    ...
    hot_numbers = [(num, count, weighted_freq.get(num, 0), time_decay_scores.get(num, 0)) for num, count in sorted_freq[:10]]
    cold_numbers = [(num, count, weighted_freq.get(num, 0), time_decay_scores.get(num, 0)) for num, count in sorted_freq[-10:]] if len(sorted_freq) >= 10 else []
...
def empty_box_predictor():
    try:
        df = load_csv_data()
        if df.empty:
            return render_template('empty_box_predictor.html', predictions=[], message="No data available", next_empty_predictions=[], draws=[], provider_options=['all'], provider='all', month_options=[], selected_month='')
...
def hot_cold():
    ...
    hot = []
    cold = []
    for num, count in sorted_freq[:20]:
        momentum = calculate_temperature_momentum(num, recent_df)
        hot.append({'number': num, 'count': count, 'momentum': momentum})
    
    for num, count in sorted_freq[-20:] if len(sorted_freq) >= 20 else []:
        momentum = calculate_temperature_momentum(num, recent_df)
        cold.append({'number': num, 'count': count, 'momentum': momentum})
    
    # Cross-provider temperature sync
    cross_provider_sync = analyze_cross_provider_sync(df, [item['number'] for item in hot[:10]])
    
    # Hot-cold transition timing
    transition_timing = analyze_transition_timing(recent_df)
    
    return render_template('hot_cold.html', hot=hot, cold=cold, neutral=[], days=days, message="", last_updated=time.strftime("%Y-%m-%d %H:%M:%S"), provider_options=provider_options, provider=provider, month_options=month_options, selected_month=selected_month, temperature_momentum=hot[:5], cross_provider_sync=cross_provider_sync, transition_timing=transition_timing)
