...
def train_model():
    """Train ML model from learned data using XGBoost"""
    if not os.path.exists(CSV_FILE):
        print("❌ No predictions file found")
        return None, None
    ...
    print(f"🧠 Training XGBoost model with {len(learned)} samples...")

    X = []
    y = []

    for idx, row in learned.iterrows():
        features = extract_ml_features(row, df)  # Pass df for advanced features
        X.append(features)
    ...
    print(f"✅ XGBoost Model trained - Accuracy: {accuracy:.2%}")

    return model, scaler

def predict_with_model(features, df=None):
    ...
    # Extract full features including advanced ones
    if df is not None:
        # Create a dummy row for feature extraction
        dummy_row = {'predicted_numbers': ''.join(map(str, features[:4])) if len(features) >= 4 else '0000'}
        full_features = extract_ml_features(dummy_row, df)
    else:
